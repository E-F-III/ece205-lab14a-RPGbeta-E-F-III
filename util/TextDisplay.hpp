...
#include <chrono>  // Required for milliseconds and steady_clock
...
namespace util {
    ...
    const char FG_BROWN[]     = "\033[38;5;94m"; // Improved 256-color Brown
     ...
     */
    inline void printColor(const std::string& text, const char* fgCode) {
        std::cout << fgCode << text << RESET;
    }

    /**
     * @brief Prints colored text with a custom background. 
     */
    inline void printColorBg(const std::string& text, const char* fgCode, const char* bgCode) {
        std::cout << bgCode << fgCode << text << RESET; 
    }

    /**
     * @brief Implements a "Typing" effect.
     */
    inline void printType(const std::string& text, int speedMs = 50) {
        for (char c : text) {
    ...
    /**
     * @brief Clears the screen and moves cursor to home.
     */
     ...
     */
    inline void showLoadingSpinner(const std::string& text, int durationMs = 1000) {
        char spinners[] = {'|', '/', '-', '\\'};
        ...
        std::cout << CLEAR_LINE; // Clean up the line after finishing
    }
    
    /**
     * @brief Progress Bar.
     */
    inline void printProgressBar(int current, int total, char barChar = '#', const char* fgCode = FG_GREEN) {
        int width = 20; 
        if (total <= 0) total = 1;
